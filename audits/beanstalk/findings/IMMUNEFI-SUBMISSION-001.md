...
`sopWell()` in LibFlood.sol swaps newly minted sopBeans for the non Bean token during Flood events. It passes `minAmountOut = 0` to the Well (line 362). There is no floor on the exchange rate. Any amount of the non Bean token, including nearly zero, is accepted.
...
1. Sells Bean into the Well (drains the non Bean token, degrades the exchange rate)
...
All three steps happen in a single atomic transaction. No mempool monitoring needed. No front running of other users' transactions. The attacker IS the caller. On Arbitrum there is no public mempool anyway (sequencer processes transactions directly).

### Multi Well Impact

Beanstalk has 6 soppable Wells. The attack applies to every single one. I tested against the two largest:

**Bean/WETH Well** (TVL ~$17,900, block 440420446):
- 23% extraction with a 15% Bean reserve front run
- 68% extraction with an 80% Bean dump (still does not revert)
- Attacker profits 475 Bean per cycle

**Bean/wstETH Well** (TVL ~$6.8M, latest block):
- 24% extraction with a 15% Bean reserve front run
- $8,137 stolen from stalkholders on a 1% SOP
- $38,533 stolen on a 5% SOP
- Attacker profits 37,152 Bean (1% SOP) or 188,349 Bean (5% SOP)

The Bean/wstETH pool is the largest Well by far. When Flood occurs, the SOP swap on this pool alone exposes tens of thousands of dollars to extraction per event. Across all 6 Wells, the total theft per Flood season is higher.
...
Stalkholders lose their SOP proceeds. This is theft of unclaimed yield.
...
Flood requires: Bean price above peg, pod rate under 5%, raining for 2+ consecutive seasons. These are designed conditions: Flood is a core Beanstalk mechanism documented in the whitepaper (Section 14, Season of Plenty). The Flood code path exists specifically because these conditions are expected to occur during normal economic cycles.

On L1 Beanstalk (pre April 2022 exploit), Flood triggered during periods when Bean exceeded peg. The Arbitrum deployment carries the same code. When the Beanstalk economy recovers and Bean trades above $1 with a low pod rate, Flood will activate. There is no code change between now and then. The `minAmountOut = 0` and spot reserve usage will be there when it does.

The vulnerability should be fixed before these conditions are reached, not after the first exploit.
...
Everything below was run against live Basin Wells on Arbitrum. All swaps go through the actual deployed Well contracts. Full source code and setup instructions are in the PoC gist.
...
**Then I checked if spot reserves are manipulable.** I swapped 1 WETH ($2,040) into the Bean/WETH Well and read reserves before and after:
...
**Then I ran the attack on the Bean/WETH Well.** I simulated a realistic SOP (5% of Bean reserves) with and without a sandwich. The attacker sells 15% of Bean reserves into the Well before the SOP, then buys Bean back after:
...
**I deployed an actual exploit contract** (`AtomicFloodExploit`) on the fork. It front runs, the SOP executes at the degraded rate, then it back runs. Same 23% extraction. No mempool monitoring needed. The contract calls `sunrise()` directly.
...
**Then I tested the Bean/wstETH Well** ($6.8M TVL, the largest pool). Same vulnerability, much higher dollar impact:

```
=== 1% SOP ===
fair wstETH to stalkholders:     14036058553937356950 (~$33,504)
attacked wstETH to stalkholders: 10627000854122749698 (~$25,366)
stalkholder loss:                $8,137
extraction rate:                 24%
attacker Bean profit:            37,152
...
- Bean/wstETH Well: 0xBEa00BbE8b5da39a3F57824a1a13Ec2a8848D74F
