...
library CallWithExactGas {
    ...
    /// @dev Does not revert on insufficient gas. Instead, returns sufficientGas = false
    /// if there is not enough gas available for the call. This differs from other functions
    /// in this library which revert on insufficient gas.
    /// @dev Ignores the return data, which makes it immune to gas bomb attacks.
    /// @return success whether the call succeeded
    /// @return sufficientGas Whether there was enough gas to make the call (false if insufficient gas)
